...
//};
//
//struct PlayerIO { bool d = false, l = false, r = false, u = false, space = false; };
//
//struct PlayerData
//{
//    uint32_t id = 0;
...
//};
//
//struct sMessageFromClient { uint32_t nUniqueID = 0; TEXT message = TEXT::POOPSIE; };
//
...
//    msg << v.u << v.r << v.l << v.d << v.space; return msg;
//}
//inline cnet::message<Msg>& operator>>(cnet::message<Msg>& msg, PlayerIO& v) {
//    msg >> v.space >> v.d >> v.l >> v.r >> v.u; return msg;
//}
//
//// ---- sMessageFromClient ----
//inline cnet::message<Msg>& operator<<(cnet::message<Msg>& msg, const sMessageFromClient& v) {
//    msg << to_u32(v.message) << v.nUniqueID; return msg;
//}
//inline cnet::message<Msg>& operator>>(cnet::message<Msg>& msg, sMessageFromClient& v) {
//    uint32_t t = 0; msg >> t >> v.nUniqueID; v.message = to_text(t); return msg;
//}
//
//// ---- sPlayerDescription ----
//inline cnet::message<Msg>& operator<<(cnet::message<Msg>& msg, const sPlayerDescription& p) {
//    msg << p.vVel.second << p.vVel.first << p.vPos.second << p.vPos.first
//        << p.fRadius << p.nDeaths << p.nKills << p.nAmmo << p.nHealth << p.nAvatarID << p.nUniqueID;
//    return msg;
//}
//inline cnet::message<Msg>& operator>>(cnet::message<Msg>& msg, sPlayerDescription& p) {
//    msg >> p.nUniqueID >> p.nAvatarID >> p.nHealth >> p.nAmmo >> p.nKills >> p.nDeaths
//        >> p.fRadius >> p.vPos.first >> p.vPos.second >> p.vVel.first >> p.vVel.second;
//    return msg;
//}
//
//// ---- PlayerData ----
//inline cnet::message<Msg>& operator<<(cnet::message<Msg>& msg, const PlayerData& d) {
//    msg << d.frameIndex << to_u32(d.dir) << to_u32(d.animID)
//        << d.yvel << d.xvel << d.ypos << d.xpos << d.avatarID << d.id;
//    return msg;
//}
//inline cnet::message<Msg>& operator>>(cnet::message<Msg>& msg, PlayerData& d) {
//    uint32_t a = 0, di = 0;
//    msg >> d.id >> d.avatarID >> d.xpos >> d.ypos >> d.xvel >> d.yvel >> a >> di >> d.frameIndex;
//    d.animID = to_anim(a); d.dir = to_dir(di); return msg;
//}
//
//// ---- PlayerDrawData ----
//inline cnet::message<Msg>& operator<<(cnet::message<Msg>& msg, const PlayerDrawData& d) {
//    msg << d.frameIndex << to_u32(d.dir) << to_u32(d.animID) << d.ypos << d.xpos << d.id; return msg;
//}
