    ...
    void Simulate(float dt)
    {
        ...
        for (auto& kv : playerDataMap)
        {
            ...
            if (playerIOMap[kv.first].space)
            {
                kv.second.xvel = 0.f;
                kv.second.yvel = 0.f;
                if (playerDataMap[kv.first].animID != AnimID::Attack)
                {
                   
                    playerDataMap[kv.first].animID = AnimID::Attack;
                    playerDataMap[kv.first].frameIndex = 0;
                }
            }
            else
            {
                if (playerDataMap[kv.first].animID == AnimID::Attack)
                {
                    if (playerDataMap[kv.first].frameIndex >= 17)
                    {
                        playerDataMap[kv.first].frameIndex = 0;
                        if (playerIOMap[kv.first].d || playerIOMap[kv.first].r || playerIOMap[kv.first].l || playerIOMap[kv.first].u)
                        {
                            playerDataMap[kv.first].animID = AnimID::Run;
                        }
                        else
                        {
                            playerDataMap[kv.first].animID = AnimID::Idle;
                        }
                    }
                }
            }
